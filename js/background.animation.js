/*
*   http://codepen.io/Yakudoo/pen/NqKvRY
*/
...
var gui = new dat.GUI();
gui.width = 300;
gui.add(this, 'triW').min(10).max(50).step(1).name('Triangle Width').onFinishChange(start);
gui.add(this, 'triH').min(10).max(50).step(1).name('Triangle Height').onFinishChange(start);
gui.add(this, 'trailMaxLength').min(2).max(40).step(1).name('Trail Size').onFinishChange(start);
gui.add(this, 'trailIntervalCreation').min(100).max(1000).step(100).name('Creation delay').onFinishChange(start);

gui.add(this, 'speedTriOpen').min(.5).max(2).step(.1).name('Triangle open speed').onFinishChange(start);
gui.add(this, 'speedTrailAppear').min(.01).max(1).step(.01).name('Appear duration').onFinishChange(start);
gui.add(this, 'speedTrailDisappear').min(.01).max(1).step(.01).name('Disappear duration').onFinishChange(start);

gui.add(this, 'delayBeforeDisappear').min(.02).max(4).step(.01).name('Disappear delay').onFinishChange(start);
gui.add(this, 'randomAlpha').name('Random Opacity').onFinishChange(start);
gui.add(this, 'pause').name('Pause');
gui.add(this, 'closeAll').name('closeAll');
gui.add(this, 'kill').name('Kill');
gui.add(this, 'start').name('Restart');
*/
